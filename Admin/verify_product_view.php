...
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM product WHERE status = 'verified'";
if ($startDate && $endDate) {
...
// Fetch products with status 'verified' along with vendor name, category, and brand names
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 style="font-size: 25px;">Verified Products</h1>
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <form action="verify_product_view.php" method="GET" class="w-100" onsubmit="return validateDates()">
...
while ($category = $categoriesResult->fetch_assoc()) {
    ...
    $productCountQuery = "SELECT COUNT(*) AS count FROM product WHERE category_id = $categoryId AND status = 'verified'";
...
while ($brand = $brandsResult->fetch_assoc()) {
    ...
    $productCountQuery = "SELECT COUNT(*) AS count FROM product WHERE brand_id = $brandId AND status = 'verified'";
    $productCountResult = $conn->query($productCountQuery);
    $productCount = $productCountResult->fetch_assoc()['count'];
    ...
    <?php if ($totalPages > 1): ?>
        <ul class="pagination justify-content-end">
            ...
            <li class="page-item <?= $page > 1 ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page > 1 ? 'verify_product_view.php?page=' . ($page - 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Previous</a>
            </li>
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="verify_product_view.php?page=<?= $i ?>&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>

            <!-- Next page link -->
            <li class="page-item <?= $page < $totalPages ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page < $totalPages ? 'verify_product_view.php?page=' . ($page + 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Next</a>
            </li>
        </ul>
    <?php endif; ?>
</nav>
