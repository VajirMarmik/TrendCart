...
$pdf->SetAuthor('Admin');
$pdf->SetTitle('TrendCart - Verification Pending Vendors Report');
...
$sql .= " ORDER BY created_at DESC";  // Sort results in descending order by creation date
...
if ($startDate && $endDate) {
    $title = 'Verification Pending Vendors Report - Of ' . htmlspecialchars($startDate) . ' To ' . htmlspecialchars($endDate);
} else {
    $title = 'Verification Pending Vendors Report - All Verification Pending Vendors';
}
...
$pdf->Output('TrendCart - Verification Pending Vendors Report.pdf', 'D');
?>
