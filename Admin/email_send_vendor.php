<?php 
    ...
    $totalRecordsQuery = "SELECT COUNT(*) AS total FROM vendors WHERE `verification_status` = 'Verified'";
    if ($startDate) {
    ...
    $sql = "SELECT * FROM `vendors` WHERE `verification_status` = 'Verified'";
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 style="font-size: 25px;">Verified Vendor</h1>
    <a href="generate_Vendor_Verify_pdf.php?start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>" class="btn btn-secondary ms-2">
        <i class="fas fa-download me-2"></i> Download PDF
    </a>
</div>

<div class="d-flex justify-content-between align-items-center mb-3">
    <form id="reportForm" action="email_send_vendor.php" method="GET" class="w-100">
...
$totalVendorsQuery = "SELECT COUNT(*) AS total FROM vendors WHERE`verification_status` = 'Verified'";
...
$planQuery = "SELECT make_plan, COUNT(*) AS count FROM vendors WHERE`verification_status` = 'Verified' GROUP BY make_plan";
    ...
    <?php if ($totalPages > 1): ?>
        <ul class="pagination justify-content-end">
            ...
            <li class="page-item <?= $page > 1 ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page > 1 ? 'email_send_vendor.php?page=' . ($page - 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Previous</a>
            </li>
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="email_send_vendor.php?page=<?= $i ?>&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>

            <!-- Next page link -->
            <li class="page-item <?= $page < $totalPages ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page < $totalPages ? 'email_send_vendor.php?page=' . ($page + 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Next</a>
            </li>
        </ul>
    <?php endif; ?>
</nav>
