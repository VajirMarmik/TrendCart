<?php 
    ...
    $totalRecordsQuery = "SELECT COUNT(*) AS total FROM vendors WHERE `is_verified` = 'Not Joined'";
    if ($startDate) {
    ...
    $sql = "SELECT * FROM `vendors` WHERE `is_verified` = 'Not Joined'";
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 style="font-size: 25px;">Vendor Not Joined</h1>
    <a href="generate_Vendor_Not_Joined_pdf.php?start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>" class="btn btn-secondary ms-2">
        <i class="fas fa-download me-2"></i> Download PDF
    </a>
</div>

<div class="d-flex justify-content-between align-items-center mb-3">
    <form id="reportForm" action="email_not_verify_vendor.php" method="GET" class="w-100">
...
// Fetch the count of vendors per subscription plan dynamically, where is_verified = 'Not Joined'
$planCounts = [];
$planPercentages = [];

// Total vendors count with is_verified = 'Not Joined'
$totalVendorsQuery = "SELECT COUNT(*) AS total FROM vendors WHERE is_verified = 'Not Joined'";
$totalVendorsResult = $conn->query($totalVendorsQuery);
$totalVendors = $totalVendorsResult->fetch_assoc()['total'];

// Get count for each plan, filtering by is_verified = 'Not Joined'
$planQuery = "SELECT make_plan, COUNT(*) AS count FROM vendors WHERE is_verified = 'Not Joined' GROUP BY make_plan";
    ...
    <?php if ($totalPages > 1): ?>
        <ul class="pagination justify-content-end">
            ...
            <li class="page-item <?= $page > 1 ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page > 1 ? 'email_not_verify_vendor.php?page=' . ($page - 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Previous</a>
            </li>
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="email_not_verify_vendor.php?page=<?= $i ?>&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>

            <!-- Next page link -->
            <li class="page-item <?= $page < $totalPages ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page < $totalPages ? 'email_not_verify_vendor.php?page=' . ($page + 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Next</a>
            </li>
        </ul>
    <?php endif; ?>
</nav>
