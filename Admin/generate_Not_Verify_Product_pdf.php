...
$pdf->SetAuthor('Admin');
$pdf->SetTitle('TrendCart - Not Verified Products Report');
$pdf->SetMargins(15, 18, 15);
$pdf->SetAutoPageBreak(TRUE, 20);
$pdf->AddPage();
$pdf->SetFont('helvetica', '', 12);

// Fetch products with vendor names
include "db1.php";
...
if ($startDate && $endDate) {
    $sql .= " AND DATE(p.created_at) BETWEEN '$startDate' AND '$endDate'"; // Specify 'p.' for the product table
}
...
if ($result === FALSE) {
    ...
    if ($startDate && $endDate) {
        $title = 'Not-Verified Products Report - Of ' . htmlspecialchars($startDate) . ' To ' . htmlspecialchars($endDate);
    } else {
        $title = 'Not-Verified Products Report - All Not-Verified Products';
    }

    $html = '<h3>' . $title . '</h3>';

include "generate_Product_PDF_View.php";
...
$pdf->Output('TrendCart - Not Verified Products Report.pdf', 'D');
?>
