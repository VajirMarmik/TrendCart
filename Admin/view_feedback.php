...
<head>
    ...
    <title>TrendCart - View Feedback</title>
    <style>
        .download-btn {
            float: right;
            margin: 20px;
    ...
    <div class="main p-3">
        ...
        <div class="container mt-3">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h1 style="font-size: 25px;">View Feedback</h1>
                <a href="generate_View_Feedback_pdf.php?start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>" class="btn btn-secondary ms-2">
                    <i class="fas fa-download me-2"></i> Download PDF
                </a>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-3">
                <form id="reportForm" action="view_feedback.php" method="GET" class="w-100">
            ...
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        // Loop through view all feedback records and display them
                        if ($result->num_rows > 0) {
                            ...
                            while ($user = mysqli_fetch_assoc($result)) {
                                ...
                                echo "<td>" . htmlspecialchars($user['message']) . "</td>";
                                echo "<td>" . htmlspecialchars($user['created_at']) . "</td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='6' class='text-center'>No Feedback for selected date range</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
                    ...
                    <ul class="pagination justify-content-end">
                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                <a class="page-link" href="view_feedback.php?start_date=<?= htmlspecialchars($startDate) ?>&end_date=<?= htmlspecialchars($endDate) ?>&page=<?= $i ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
                    </ul>
                <?php endif; ?>
