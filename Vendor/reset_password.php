...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password !== $confirm_password) {
        ...
        // Verify the token
        $query = "SELECT email FROM vendors WHERE reset_token = ? AND reset_status = 'not reset'";
        ...
        if ($result->num_rows > 0) {
            // Token is valid, update the password and reset_status
            $email = $result->fetch_assoc()['email'];
            $update_query = "UPDATE vendors SET password = ?, reset_token = NULL, reset_status = 'reset' WHERE email = ?";
...
<head>
    ...
    <title>TrendCart - Vendor Reset Password</title>
    <style>
        body {
