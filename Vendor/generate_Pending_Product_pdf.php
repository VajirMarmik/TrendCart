...
$pdf->SetAuthor('Admin');
$pdf->SetTitle('TrendCart - '. $vendorUsername ."'s Pending Products Report");
...
$sql = "SELECT p.*, c.name AS category_name, b.name AS brand_name, v.username AS vendor_name
        ...
        WHERE p.vendor_id = $vendorId AND p.status = 'pending'";
...
$pdf->Output('TrendCart - '. $vendorUsername ."'s Pending Products Report.pdf", 'D');
?>
