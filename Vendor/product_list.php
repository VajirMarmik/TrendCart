...
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM product WHERE vendor_id = $vendorId";
if ($startDate && $endDate) {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="fs-3">Manage Product</h1>
    <div>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addProductModal">
            <i class="fas fa-plus me-2"></i> Add Product
        </button>
        <a href="generate_Product_List_pdf.php?start_date=<?= htmlspecialchars($startDate) ?>&end_date=<?= htmlspecialchars($endDate) ?>" class="btn btn-secondary ms-2">
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <form action="product_list.php" method="GET" class="w-100" onsubmit="return validateDates()">
    ...
    <?php if ($totalPages > 1): ?>
        <ul class="pagination justify-content-end">
            ...
            <li class="page-item <?= $page > 1 ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page > 1 ? 'product_list.php?page=' . ($page - 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Previous</a>
            </li>
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="product_list.php?page=<?= $i ?>&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>

            <!-- Next page link -->
            <li class="page-item <?= $page < $totalPages ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page < $totalPages ? 'product_list.php?page=' . ($page + 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Next</a>
            </li>
        </ul>
    <?php endif; ?>
</nav>
