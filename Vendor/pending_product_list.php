...
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM product WHERE vendor_id = $vendorId && status = 'pending'";
if ($startDate && $endDate) {
...
$sql = "SELECT p.*, c.name AS category_name, b.name AS brand_name 
        ...
        WHERE p.vendor_id = $vendorId && p.status = 'pending'";
...
<?php include "product_header.php"; ?>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 style="font-size: 28px;">Pending Products</h1>
        <div>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addProductModal">
                <i class="fas fa-plus me-2"></i> Add Product
            </button>
            <a href="generate_Pending_Product_pdf.php?start_date=<?= htmlspecialchars($startDate) ?>&end_date=<?= htmlspecialchars($endDate) ?>" class="btn btn-secondary ms-2">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <form action="pending_product_list.php" method="GET" class="w-100" onsubmit="return validateDates()">
...
while ($category = $categoriesResult->fetch_assoc()) {
    ...
    $productCountQuery = "SELECT COUNT(*) AS count FROM product WHERE category_id = $categoryId AND vendor_id = $vendorId AND status = 'pending'";
...
while ($brand = $brandsResult->fetch_assoc()) {
    ...
    $productCountQuery = "SELECT COUNT(*) AS count FROM product WHERE brand_id = $brandId AND vendor_id = $vendorId AND status = 'pending'";
    $productCountResult = $conn->query($productCountQuery);
    $productCount = $productCountResult->fetch_assoc()['count'];
    ...
    <?php if ($totalPages > 1): ?>
        <ul class="pagination justify-content-end">
            ...
            <li class="page-item <?= $page > 1 ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page > 1 ? 'pending_product_list.php?page=' . ($page - 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Previous</a>
            </li>
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="pending_product_list.php?page=<?= $i ?>&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>

            <!-- Next page link -->
            <li class="page-item <?= $page < $totalPages ? '' : 'disabled' ?>">
                <a class="page-link" href="<?= $page < $totalPages ? 'pending_product_list.php?page=' . ($page + 1) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate) : '#' ?>">Next</a>
            </li>
        </ul>
    <?php endif; ?>
</nav>
